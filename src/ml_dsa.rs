    ...
    let (t_1, t_0): ([R; K], [R; K]) = {
        ...
        let t: [R; K] = core::array::from_fn(|k| {
            R(core::array::from_fn(|n| {
                full_reduce32(t_not_reduced[k].0[n])
            }))
        });
        power2round(&t)
    };
    ...
    let t1_d2_hat_mont: [T; K] = {
        ...
        to_mont(&core::array::from_fn(|k| {
            T(core::array::from_fn(|n| {
                mont_reduce(i64::from(t1_hat_mont[k].0[n]) << D)
            }))
        }))
    };
...
>(
    ...
    loop {
        ...
        z = core::array::from_fn(|l| {
            R(core::array::from_fn(|n| {
                partial_reduce32(y[l].0[n] + c_s_1[l].0[n])
            }))
        });
...
>(
    ...
    let wp_1: [R; K] = core::array::from_fn(|k| {
        R(core::array::from_fn(|n| {
            use_hint(gamma2, h[k].0[n], wp_approx[k].0[n])
        }))
    });
...
pub(crate) fn private_to_public_key<const K: usize, const L: usize>(
    ...
    let s_1_hat: [T; L] = core::array::from_fn(|l| {
        T(core::array::from_fn(|n| {
            mont_reduce(i64::from(s_1_hat_mont[l].0[n]))
        }))
    });
    ...
    let s_2: [R; K] = inv_ntt(&core::array::from_fn(|k| {
        T(core::array::from_fn(|n| {
            mont_reduce(i64::from(s_2_hat_mont[k].0[n]))
        }))
    }));
    ...
    let t_0: [R; K] = inv_ntt(&core::array::from_fn(|k| {
        T(core::array::from_fn(|n| {
            mont_reduce(i64::from(t_0_hat_mont[k].0[n]))
        }))
    }));
...
pub(crate) fn encode_public<const K: usize, const L: usize, const PK_LEN: usize>(
    ...
    let t1_d2: [R; K] = inv_ntt(&core::array::from_fn(|k| {
        T(core::array::from_fn(|n| {
            mont_reduce(i64::from(t1_d2_hat_mont[k].0[n]))
        }))
    }));
...
>(
    ...
    let PrivateKey {
        ...
        cap_k,
        tr,
        s_1_hat_mont: s_hat_1_mont,
        s_2_hat_mont: s_hat_2_mont,
        t_0_hat_mont: t_hat_0_mont,
        ..
    } = sk;

    // mont->norm each n coeff, of L entries of T, then inverse NTT
    let s_1: [R; L] = inv_ntt(&core::array::from_fn(|l| {
        T(core::array::from_fn(|n| {
            mont_reduce(i64::from(s_hat_1_mont[l].0[n]))
        }))
    }));
    // correct each coeff such that they are centered around 0
    let s_1: [R; L] = core::array::from_fn(|l| {
        R(core::array::from_fn(|n| {
            if s_1[l].0[n] > (Q / 2) {
                s_1[l].0[n] - Q
            } else {
    ...
    let s_2: [R; K] = inv_ntt(&core::array::from_fn(|k| {
        T(core::array::from_fn(|n| {
            mont_reduce(i64::from(s_hat_2_mont[k].0[n]))
        }))
    }));
    let s_2: [R; K] = core::array::from_fn(|k| {
        R(core::array::from_fn(|n| {
    ...
    let t_0: [R; K] = inv_ntt(&core::array::from_fn(|k| {
        T(core::array::from_fn(|n| {
            mont_reduce(i64::from(t_hat_0_mont[k].0[n]))
        }))
    }));
    let t_0: [R; K] = core::array::from_fn(|k| {
        R(core::array::from_fn(|n| {
            if t_0[k].0[n] > (Q / 2) {
                t_0[k].0[n] - Q
            } else {
    ...
    // Encode and return
    sk_encode::<K, L, SK_LEN>(ETA, rho, cap_k, tr, &s_1, &s_2, &t_0)
}

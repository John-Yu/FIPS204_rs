...
use fips204_rs::{KeyGen, MlDsa44, MlDsa65, MlDsa87, Signer, Verifier};
...
impl RngCore for TestRng {
    ...
    fn try_fill_bytes(&mut self, out: &mut [u8]) -> Result<(), rand_core::Error> {
        out.iter_mut()
            .for_each(|b| *b = self.value.to_le_bytes()[0]);
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let (pk44, sk44) = MlDsa44::try_keygen_with_rng(&mut rng).unwrap();
    let sig44 = sk44.try_sign(&msg, &[0]).unwrap();

    let (pk65, sk65) = MlDsa65::try_keygen_with_rng(&mut rng).unwrap();
    let sig65 = sk65.try_sign(&msg, &[0]).unwrap();

    let (pk87, sk87) = MlDsa87::try_keygen_with_rng(&mut rng).unwrap();
    let sig87 = sk87.try_sign(&msg, &[0]).unwrap();

    c.bench_function("ml_dsa_44 keygen", |b| {
        b.iter(|| MlDsa44::try_keygen_with_rng(&mut rng))
    });
    c.bench_function("ml_dsa_65 keygen", |b| {
        b.iter(|| MlDsa65::try_keygen_with_rng(&mut rng))
    });
    c.bench_function("ml_dsa_87 keygen", |b| {
        b.iter(|| MlDsa87::try_keygen_with_rng(&mut rng))
    });
    ...
    c.bench_function("ml_dsa_44 pk verify", |b| {
        b.iter(|| pk44.verify(&msg, &sig44, &[]))
    });
    c.bench_function("ml_dsa_65 pk verify", |b| {
        b.iter(|| pk65.verify(&msg, &sig65, &[]))
    });
    c.bench_function("ml_dsa_87 pk verify", |b| {
        b.iter(|| pk87.verify(&msg, &sig87, &[]))
    });
}
